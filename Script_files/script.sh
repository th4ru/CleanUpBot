...
confirm_clean() {
    if [[ $MODE == "interactive" ]]; then
        echo -e "\n${RED}${BOLD}â ï¸  WARNING: This will permanently delete files from your system${NC}"
        echo -e "${RED}${BOLD}â ï¸  NO SIZE LIMITS - Files of ANY size will be deleted!${NC}"
...
cleanup() {
    ...
    if [[ $ERRORS -gt 0 ]]; then
        echo -e "${RED}There were $ERRORS errors during cleanup. Check the log for details.${NC}"
    fi
}
